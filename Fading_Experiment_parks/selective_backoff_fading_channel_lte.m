function backoff_fading_channel_lte()
        ...
        for t=1:2
            ...
            while (ind < (repn+1))   %%%%% for loop with different packet loss percentage %%%%
                ...
                try
                    ...
                    fprintf(fvid, '\n ******* Simulation at [Iteration: %d, repeat: %d] for Tx probability = %0.2f ******** \n', it, repeat, tx_prob); 
                    ...
                    while (ischar(tline)) 
                        ...
                        if(frame_type ~= 0)
                            ...
                            else   %%if policy=2 for backoff
                                if(curr_frame == 1) %% I frame
                                    d2d_idle(next_state_u, next_state_i);
                                else
                                    if(failure_flag == 1)
                                        if(curr_frame == prev_frame) %% same frame continues
                                            if(curr_frame == 2) %% B/P frames
                                                if(delta_slot > 0)
                    ...
                    if(repeat == MAX_REPEAT)
                        % Fillup the array for the iteration for this ind %
                        getCumAvg(var);
                        %calculate and fill cAvg
                        if(cAvgPsnr(ind) == 0)
                            cAvgPsnr(ind) = localCumPsnr;
                        else
                            if(localCumPsnr ~= 0)
                               cAvgPsnr(ind) = ((it-1)*cAvgPsnr(ind) + localCumPsnr)/it;
                               fprintf(fvid, '\t Average cumulative PSNR for Tx_prob: %.3f = %.3f \n', tx_prob, cAvgPsnr(ind));
                            end
                        end
                        
                        cAvgD2dThrpt(ind) = ((it-1)*cAvgD2dThrpt(ind) + localCumd2dTrhpt)/it;
                        fprintf(fvid, '\t Cumulative average D2D Throughput percentage at repeat %d for Tx_prob: %.3f = %.3f \n', repeat, tx_prob, localCumd2dTrhpt);

                        if(localCumDetProb ~= 0)
                           cAvgDetProb(ind) = ((it-1)*cAvgDetProb(ind) + localCumDetProb)/it;
                           fprintf(fvid, '\t Average cumulative Detection Probability for Tx_prob: %.3f = %.3f \n', tx_prob, cAvgDetProb(ind));
                        end

                        if(localCumTotDetProb ~= 0)
                           cTotDetProb(ind) = ((it-1)*cTotDetProb(ind) + localCumTotDetProb)/it;
    ...
    function calculateMetrics(ind)                
        ...
        if(localCumd2dTrhpt ==0)
            localCumd2dTrhpt = t_dd;
        else
            localCumd2dTrhpt = ((repeat-1)*localCumd2dTrhpt + t_dd)/repeat;
        end
        %cAvgD2dThrpt(ind) = ((it-1)*cAvgD2dThrpt(ind) + t_dd)/it;
        fprintf(fvid, '\t D2D Throughput percentage at repeat %d for Tx_prob: %.3f at it : %d = %.3f \n', repeat, tx_prob, it, t_dd);
        fprintf(fvid, '\t Cumulative average D2D Throughput percentage at repeat %d for Tx_prob: %.3f = %.3f \n', repeat, tx_prob, localCumd2dTrhpt);
        
       
        fprintf('Now Calculate the PSNR w.r.t damaged packets \n');
        [m_psnr] = parks_psnr(badpackets_ref, badpackets);
        ...
        if(localCumPsnr == 0)
            ...
            if(m_psnr ~= 0)
               localCumPsnr = ((repeat-1)*localCumPsnr + m_psnr)/repeat;
               fprintf(fvid, '\t Average cumulative PSNR at repeat: %d for Tx_prob: %.3f = %.3f \n', repeat, tx_prob, localCumPsnr);
            end
        end
        ...
        fprintf(fvid, '\t Avg. object detection probability at repeat: %d for Tx_prob: %.3f at iteration: %d = %.3f \n', repeat, tx_prob, it, detection_prob);
        fprintf(fvid, '\t Total object detection probability at repeat: %d for Tx_prob: %.3f at iteration: %d = %.3f \n', repeat, tx_prob, it, detTotProb);
        
        if(localCumDetProb == 0)
            localCumDetProb = detection_prob;
        else
            if(detection_prob ~= 0)
               localCumDetProb = ((repeat-1)*localCumDetProb + detection_prob)/repeat;
               fprintf(fvid, '\t Average cumulative Detection Probability at repeat: %d for Tx_prob: %.3f = %.3f \n', repeat, tx_prob, localCumDetProb);
            end
        end
        
        if(localCumTotDetProb == 0)
            localCumTotDetProb = detTotProb;
        else
            if(detTotProb ~= 0)
               localCumTotDetProb = ((repeat-1)*localCumTotDetProb + detTotProb)/repeat;
               fprintf(fvid, '\t Total cumulative Detection Probability at repeat: %d for Tx_prob: %.3f = %.3f \n', repeat, tx_prob, localCumTotDetProb);
            end
        end
